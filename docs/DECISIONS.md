...
- **Update 2026-09-08:** Play's sign-in poll was calling the same start path that kills a denied client. A leftover `Account Logon Denied` from the previous session stayed in `console_log.txt` while Steam was applying its own update, so every 2-second poll killed the updater window and spawned another `-login`. Polling is now read-only. An in-progress `bootstrap_log.txt` download is treated as updating, not as a failed login, and Steam is left running until it finishes.
