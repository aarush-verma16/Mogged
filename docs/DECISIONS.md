...
- **Cost:** The password reaches Steam as a process argument, readable by other processes on this Mac for the life of the sign-in — confirmed via `ps`/`pgrep` during testing. Sign-in also depends on a Guard code being current. If Steam ever demands interactive confirmation we cannot satisfy, the shim becomes unavoidable.
- **Update 2026-09-05:** SteamCMD's device trust (used for Install) does not carry over to the graphical client — the first `-login` on a fresh prefix comes back `Account Logon Denied` and wants its own one-time code, even for an account SteamCMD already trusts. Mogged reads the client's `console_log.txt` (`SteamServices.needsGuardCode`) to notice this, kills the stuck client — including one left running from a previous app session, matched by this prefix's own `steam.exe` path so it can never touch another title or the real Steam — and asks for a fresh code in the same account fields Install uses, rather than retrying the same denied login forever.
- **Update 2026-09-05 (second pass):** the password-as-process-argument cost above is not theoretical — it was read directly from `ps`/`pgrep` output twice while debugging this feature, in plaintext, in an agent's own tool output. That is a standing risk on any shared or multi-user Mac for as long as this design holds; do not add further tooling that lists full process command lines for Steam or the game without redacting `-login` arguments first. Also found in this pass: quitting Mogged did not stop the game or a Steam client it started, so a denied login kept retrying as a detached orphan indefinitely, surfacing its window with no connection to a Play click. Fixed with `RuntimeSupervisor.stopAll()` on quit and an orphan sweep on every app start — the sweep initially did nothing because `pkill -f` parses its pattern as a POSIX ERE, and "Program Files (x86)" contains unescaped metacharacters that make an unescaped path match nothing.
