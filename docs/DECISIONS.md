...
- **Update 2026-09-08 (one code):** `steam.exe -login` does not accept a Guard code — that is SteamCMD-only. Mogged had been appending the code anyway; Steam ignored it, emailed a new one, and the code the player just pasted was already stale. Play now runs `steamcmd.exe` in the same folder (`+login user password code`), which writes device trust (`ssfn*`). After that, `steam.exe` starts with no code and should not ask again.
