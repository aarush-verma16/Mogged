...
- **Update 2026-09-08 (code submit):** Play is the only way to send a Guard code. A leftover `Account Logon Denied` from the first attempt was still the live log when Play was pressed again, so Mogged aborted before Steam could use the new code. Submitting a code now archives that log (`SteamServices.beginLoginAttempt`) and starts a fresh `-login` with the compact uppercase code from the account fields.
