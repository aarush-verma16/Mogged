...
Living snapshot. Last updated: 2026-09-08. If this disagrees with code, fix this file in the same change.
...
- **Play no longer kills Steam while it is updating.** 2026-09-08: the sign-in poll was treating a previous session's `Account Logon Denied` as a reason to kill and relaunch every 2 seconds. Steam's updater window opened, then vanished, in a loop. Polling is now read-only; an in-progress client update is waited out.
